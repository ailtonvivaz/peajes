	...
	return {
		...
		updateTheme: function () {
			...
			if (newTheme && newTheme != this.theme) {
				this.theme = newTheme;
				var theme_el = gx.dom.byId(GX_THEME_CSS_ELEMENT);
				if (theme_el)
		...
		dom: {
			...
			hasAnimation: function (el) {
				if (el && Modernizr.csstransitions)
				{
					...
					return gx.dom.getComputedStyle(el)[property] && gx.dom.getComputedStyle(el)[property] != 'none';
				}
				return false;
			},
			...
			fixes: (function () {
				var TABLE_RESETS_STYLE_ID = "gx-table-resets-fix",
				...
				return {
					html5: function (root) {
						...
						for (i = tables.length - 1; i >= 0; i -= 1) {
							...
							if (cellpadding) {
								table.setAttribute("cellPadding", parseInt(cellpadding, 10));
							}
							var cellspacing = table.getAttribute("data-cellspacing");
							if (cellspacing) {
								table.setAttribute("cellSpacing", parseInt(cellspacing, 10));
							}
						...
						if (isIE || browser.isOpera()) {
							...
							for (i = 0, len = cells.length; i < len; i++) {
								...
								if (dataAlign) {
									...
									cells[i].style.textAlign = dataAlign;
								}
							}
						}
					},
					...
					popupMinWidth: function () {
						if (location.search.indexOf("gxPopupLevel") >= 0 && gx.runtimeTemplates) {
					...
					fixTableResets: function (root) {
						...
						var styleEl = $("#" + TABLE_RESETS_STYLE_ID)[0];
						if (!styleEl) {
							styleEl = document.createElement("style");
							styleEl.id = TABLE_RESETS_STYLE_ID;
							document.head.insertBefore(styleEl, gx.dom.byId(GX_THEME_CSS_ELEMENT));
						}
						...
						var cellPaddingsArray = $.map(allCellPaddings, fnObjToArray)
						var cellSpacingArray = $.map(allCellSpacings, fnObjToArray);
		...
		evt: {
			...
			onfocus_impl: function (Ctrl, gxFocusCtrl, gxWCP, gxInMasterPage, gxCurrentRow, gxCurrentGrid, gxAddLines) {
				try {
					...
					gx.csv.checkRowChange(lastId, gridChange, gxO, lastRow).done(function (rowValidated) {
						...
						if ((!rowValidated || gridChange) && (gxO.focusControl >= lastId || goForwardInGrids)) {
			...
			dispatcher: {
				...
				cleanEventName: function(dirtyEventName, gxO) {
					...
					return (tmpName && tmpName.length && tmpName.length > 1) ? tmpName[1].replace(/\.\d*$/,'') : dirtyEventName;
				},
				...
				dispatch: function(dirtyEventName, gxO, grid, row, serverSide, evtArguments) {
					...
					if (dispatchedEvts) {
						...
						setTimeout(this.dispatchTimeoutCallback.closure(this, [contextKey]), this.EXECUTION_TIMEOUT);
					}
		...
		csv: {
			...
			validControl: function (Id, bForceCheck, gxO) {
				var deferred = $.Deferred(),
					gxO = gxO || gx.O,
					bRet = true,
					bFailedCtrl = -1,
		...
		http: {
			...
			redirect: function (url, disableFrm, forceFullGet) {				
				gx.evt.setProcessing(true);				
				var currentPopup = gx.popup.getPopup();
				if (currentPopup != null) {
					var currentLvl = currentPopup.window.gx.popup.lvl;
			...
			iframeOnload: function () {
				...
				if (iFrame) {
					...
					if (frameDoc) {
						...
						if (frameBody.childNodes.length == 1 && frameBody.childNodes[0].nodeName == "#text") {
			...
			loadStyle: function (url, callback, beforeTheme, id) {
				...
				if (style) {
					...
					if (beforeTheme) {
						var themeEl = gx.dom.byId(GX_THEME_CSS_ELEMENT);
		...
		*/
		Template: (function () {
			...
			template.prototype = {
				...
				* after creating the deafult context and before applying the template.
				...
				* must map with the name of the changed property. For example: Visible, Enabled, Caption, Class (see gx.fn.setCtrlPropertyImpl function).
				...
				* A function fired before GX standard property change routines. Is handles changes made on a property named "Property". If it returns false,
				...
				* True if the control is now visible. False if it's not visible.
				* @property listeners.after {Object)
				* A config object where the event handlers are specified.
				* @property listeners.after.Property {Function)
				* A function fired after GX standard property change routines. Is handles changes made on a property named "Property". 
	...
	return {
		...
		setInnerText: function (control, text, format, multiline) {
			if (control.nodeValue != null)
				...
				if (gx.util.browser.isFirefox() || gx.util.browser.isChrome())
	...
	suggestProvider: function (ControlId, ControlRefresh, CtrlSvc) {
		...
		this.requestSuggestions = function (oAutoSuggestControl, bTypeAhead) {
			var i,
				...
				sParms = 'gxajaxSuggest_' + this.CtrlSvc,
				len = this.ControlId.length;
	...
	return {
		...
		setJsonResponse: function (response, isPostback, afterCmpLoaded, gxO, gridId, row) {
			...
			if (gx.evt.srvCommand) {
				...
				doFunc(fn.setJsonComponents, response.gxComponents, response.DynComponentMap, response.gxHiddens, function () {
					gx.http.loadStyles();
					gx.updateTheme();
					if (isPostback) {
					...
					if (isPostback) {
						var userControls;
						if (gx.pO.supportAjaxEvents) {
							userControls = gx.lang.arrayUnique(propUCs.concat(valuesUCs));
						}
...
gx.grid = {
	...
	grid: function (parentObj, gLvl, gLvlName, gId, gName, rgName, cName, gCmpCtx, gIsInMaster, pName, gKey, gFreestyle, gCols, aSelect, aHover, gRows, gPaging, gDragable, gSetsCtx, gBondColl, gWidth, gWidthUnit, newRowtext, pageSizeParm, gHasAddlines, gHasFEL, gImplClass, gOnLineActEvt, aCollap, gBondCollName, gResponsive, gResponsiveCols) {
		...
		this.parentObject['sub' + gName + '_Rows'] = (function(parentGridRow) {
			var ownerGrid = this.parentObject.getGridById(gId, parentGridRow);
			return ownerGrid.grid.pageSize / ownerGrid.grid.gxGridCols;
		}).closure(this);
		this.parentObject[propertyValueResolverFnName('nFirstRecordOnPage')] = (function(parentGridRow) {
			var ownerGrid = this.parentObject.getGridById(gId, parentGridRow);
			return ownerGrid.grid.firstRecordOnPage;
		}).closure(this);
		this.parentObject[propertyValueResolverFnName('nEOF')] = (function(parentGridRow) {
			var ownerGrid = this.parentObject.getGridById(gId, parentGridRow);
		...
		this.callAsyncRefresh = function (refreshParms, timeout) {			
			...
			this.refreshTimer = window.setTimeout((function () {
				if (this.autoRefreshing) {
					...
					gx.fx.obs.addObserver('grid.onafterrefresh', this, function () {gx.evt.notifyRefreshGrid()}.closure(), {single:true});
		...
		this.refreshGrid = function (loadChildGrids, fromAutoRefresh, fromCollection) {
			...
			var afterRender = function () {
				...
				this.setupGridUsercontrols(this.GridUserControls);
				gx.plugdesign.applyTemplateObject("#" + this.gxContainerCtrl.id);
		...
		this.updateRcdCount = function () {
			if (this.hasForEachLine || this.parentObject.isTransaction()) {
				gx.fn.setHidden(this.grid.gxCmpContext + "nRC_GXsfl_" + this.gridId, this.lastRowId.toString());
			}
...
gx.GxObject = (function($) {
	var gxObject = function () {
			...
			try {
		        if(bc){
				    ...
				    if (gx.lang.isArray(obj) && vStruct && vStruct.grid) {
				    	obj = bc[Number(gx.fn.currentGridRow(vStruct.grid)) - 1];
				    }
				    for (var i = 0; i < len; i++) {
		...
		this.clean = function (unloadMasterPage) {
			...
			gx.cache.removeRemoteFile(this.ServerClass.toLowerCase() + ".js");
		};
